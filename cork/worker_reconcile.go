...
const (
	// managedLabel marks every container cork creates (see startContainers).
...
// reconcileWorker removes what cork created on the worker for instances it
...
// reclaimAfterGC clears what the crash-GC's instances left on their workers.
...
// as is any network that does not follow cork's naming.
