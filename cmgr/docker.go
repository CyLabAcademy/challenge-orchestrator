...
func (m *Manager) startNetwork(instance *InstanceMetadata, opts NetworkOptions) error {
	...
	if errdefs.IsConflict(err) {
		...
		// finished on the daemon after its containers were gone). It is never
		// one another launch is about to use: the one path that relaunches an
		// existing id, a rebuild's restart, runs under updateMu. Nothing of
