...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| CORK_DB                  | SQLite DB Path. The default was `./cmgr.db` before the rename and nothing moves an existing file, so a deployment that never set this either points it at the old name or starts with an empty database | ./cork.db                                                                |
...
| CORK_WORKER_CONTROL_TIMEOUT | Ceiling for one container/network call to a worker's dockerd. A call that fails at the connection level ejects the worker at once; one that hits the timeout has cork ping the daemon, which ejects it at once only if the ping goes unanswered | 30s                                     |
| CORK_WORKER_TIMEOUTS_TO_EJECT | Separate stalls, against a daemon that still answers pings, that eject a worker within CORK_WORKER_TIMEOUT_WINDOW. One alone does not: dockerd blocks every create while it deletes an evicted image, and that is slow, not gone. A stall counts once however many calls it times out, and stalls are at least one control timeout apart | 3 |
| CORK_WORKER_TIMEOUT_WINDOW | The window CORK_WORKER_TIMEOUTS_TO_EJECT counts within | 2m |
...
| CORK_BASE_PINS           | JSON map of base image reference to digest. When present, `FROM name:tag` is rewritten to the digest in the build context, so the builder never re-resolves a mutable tag. See [BUILDER.md](BUILDER.md) | \<CORK_DIR\>/.base-pins.json (commit it; see BUILDER.md)                       |
