...
# cork (corkd, cork and cork-build) built from this checkout, for the
...
# corkd). python3 runs a challenge's own solve script against a live instance
# (the remote-make step); the examples' solvers are plain stdlib. None of
# these is used by corkd itself.
